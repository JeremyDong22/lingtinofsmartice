...
### 新增 (Added)
- **反馈聚合问题追踪** — 同类顾客反馈自动聚合为"问题"，频次统计+7天趋势图，替代逐条处理578条独立反馈的模式
- **店长问题卡片** — Dashboard 新增"问题追踪"区块，支持三种分类操作（已处理/待办/忽略），待办超期提醒，证据展开+原声播放
- **厨师长厨房问题趋势** — Chef Dashboard 新增厨房相关问题聚合视图，Quick Stats（活跃问题/本周新增/趋势下降），支持语音输入处理措施
- **管理层反馈透明度** — Admin Briefing 新增跨门店问题概览，一屏扫完所有门店状态，异常高亮，可展开查看门店操作详情+回复指示
- **管理层回复闭环** — 管理层可针对具体问题发表意见/指示，门店端（店长+厨师长）收到红点提醒，点击后标记已读
- **多角色独立操作** — 同一问题支持店长和厨师长分别标记处理状态，互相可见对方操作，管理层看到完整操作链
- **实时聚合** — AI 分析完录音后立即聚合 feedbacks 到 issues，店长当天可见；21:05 cron 更新趋势数据

## [2.6.4] - 2026-03-15

### 修复 (Fixed)
- **Landing Page 移动端适配** — 闭环流程/核心能力/数据统计改为手机双列布局；SaaS 对比表支持横滑；表单输入增大触摸目标（44px）；ShareButton 适配 iPhone 安全区域与微信导航栏；去除中文 text-justify 避免字间距异常

## [2.6.3] - 2026-03-15

### 新增 (Added)
- **Landing Page V2** — 新增闭环流程展示、已验证核心能力（8 项）、价值时间线三大板块；内容脱敏优化；页面拆分为独立模块提升可维护性

## [2.6.2] - 2026-03-15

### 改善 (Changed)
- **每日复盘待办分组** — 上次待办按角色（厨师长/前厅/店长/全员）分组折叠，匹配会议逐人点名场景，每条显示类型标签

### 修复 (Fixed)
- **复盘页顶部遮挡** — 修复 header 进度条导致首行内容被遮挡不可见的问题

## [2.6.1] - 2026-03-15

### 变更 (Changed)
- **系统管理页面开放** — 用户活动、系统健康、报名管理、知识引擎 4 个页面从单人独占改为白名单控制，开放给 9 名核心管理人员

### 移除 (Removed)
- **删除 huangrui 账号** — 移除黄睿的管理员账号（无关联数据）

## [2.6.0] - 2026-03-14

### 新增 (Added)
- **引导式每日复盘** — 选择「每日复盘」会议类型后自动进入 4 步引导流程：会前简报（数据+待办+议题）→ 录音 → AI 提取行动项 → 确认提交。议题只展示现象和证据，引导团队在会议中追问根因
- **行动项批量创建** — 新增 `POST /api/action-items/batch-create` 接口，复盘确认页一次性提交所有行动项（替代逐条创建）
- **行动项增删改** — 新增创建、删除、批量确认接口，支持复盘后编辑/删除/手动添加行动项

## [2.5.4] - 2026-03-14

### 修复 (Fixed)
- **错误页面优化** — 遇到缓存冲突等异常时，显示友好的中文错误提示和「重试」「清除缓存并刷新」按钮，替代之前的白板 "Application error" 页面

## [2.5.3] - 2026-03-14

### 优化 (Changed)
- **恢复自动保存** — 移除停止录音后的确认步骤，恢复无感自动保存体验。误录可通过历史列表左滑删除

## [2.5.2] - 2026-03-14

### 修复 (Fixed)
- **录音防息屏** — 录音期间自动阻止屏幕关闭，解决华为鸿蒙系统下息屏导致录音中断的问题

## [2.5.1] - 2026-03-12

### 优化 (Changed)
- **音频播放器体验优化** — 滑块轨道加粗更易操作，未播放时也显示录音总时长

## [2.5.0] - 2026-03-12

### 新增 (Added)
- **知识引擎管理看板** — 管理端新增知识引擎看板页面，可视化展示知识条目数量、分类分布、审核状态，支持按分类筛选和搜索
- **知识审核页增强** — 新增分类 Tab 切换、来源标签、审核操作优化，管理员可高效审核 AI 提炼的知识条目
- **知识引擎菜单入口** — UserMenu 下拉菜单新增「知识引擎」入口（管理员可见）
- **Landing Page** — 新增公开落地页 + Beta 注册表单，支持 PMF 验证
- **Beta 注册管理页** — 管理端新增注册用户管理页面，查看和管理 Beta 申请

### 优化 (Changed)
- **知识引擎从纯 AI 工具升级为业务面板** — 知识条目从仅 AI 内部使用升级为管理员可查看、审核、搜索的业务工具
- **版本号更新规范** — 新增完整版本更新判断清单 + pre-commit hook 强制拦截，防止版本号遗漏

## [2.4.4] - 2026-03-12

### 修复 (Fixed)
- **会议语音进度条三连跳** — 修复 iOS 底部弹窗中拖动进度条导致页面跳动的问题，将播放器移至独立行并阻止触摸事件冒泡
- **音频播放约1分钟后中断** — Service Worker 跨域超时策略会截断 Supabase 长音频，改为 NetworkOnly 绕过缓存；增加 buffering 状态提示

## [2.4.3] - 2026-03-09

### 优化 (Changed)
- **导航切换更流畅** — 所有角色（店长/管理层/厨师长）切换页面时即刻显示骨架屏过渡，消除"卡住"感
- **导航触摸反馈** — 底部导航栏点击时有缩放+透明度反馈，触控更灵敏（含 iOS PWA 300ms 延迟优化）

### 修复 (Fixed)
- **Activity 页面配色统一** — 资源标签、进度条、状态 badge 改为灰+indigo 克制配色，与 briefing 等管理端页面视觉一致
...
### 新增 (Added)
- **品牌 KPI 指标栏** — 品牌行下方新增 4 个核心指标（满意度/老客占比/复盘率/解决率），大号数字直观展示品牌健康度
- **老客占比指标** — brand-kpi API 新增 `summary_kpi`（avg_sentiment / repeat_ratio / resolution_rate）

### 优化 (Changed)
- **暖色系统一配色** — 趋势图、问题分布、行动老化统一使用暖色系（珊瑚→桃→杏→沙），提升设计一致性
- **问题分布改为独立条形** — 每个分类单独一行，带浅色底条和彩色标签，更清晰直观
- **行动老化改为三格卡片** — 独立计数卡片替代堆叠条，直观展示各时段分布
- **品牌行简化** — 品牌行头部简化为品牌名+门店数，KPI 数据下移到指标栏

## [2.4.0] - 2026-03-07

### 新增 (Added)
- **品牌可视化看板** — 管理端总览页品牌行下方新增 4 个可视化：满意度 30 天趋势、解决率 30 天趋势、问题分类分布、行动项老化分布
- **品牌 KPI API** — 新增 `/api/dashboard/brand-kpi` 端点，返回品牌级满意度趋势、解决率趋势、问题分布和行动老化数据
- **SVG 迷你趋势图** — 纯 SVG 实现的面积趋势图组件（无外部图表库），带渐变填充和健康色自适应

### 优化 (Changed)
- **总览页信息层级重构** — 移除底部重复的全局 2×2 KPI 网格，结果指标（满意度/老客占比/复盘率/解决率）已提升至品牌 KPI bar，减少信息冗余

## [2.3.0] - 2026-03-07

### 新增 (Added)
- **问卷调查 v3** — 问卷措辞从指令式升级为对话式，更贴近自然交流语境；新增来源追踪字段
- **执行追踪面板** — 店长端看板 + 管理端总览新增每日执行数据卡（桌访数/复盘率/连续复盘天数）
- **总览 3 级展开** — 管理端总览页品牌→门店→问题三级折叠结构，替代原门店网格
- **反馈摘要横幅** — 店长看板顶部结构化摘要（问题数+桌数+Top 关键词 / 好评数 / 建议数）
- **复盘闭环卡片** — 店长看板新增 DailyReviewCard，展示当日复盘状态和待办跟进
- **洞察品牌分组** — 管理端洞察页顾客反馈和画像 Tab 按品牌分组展示
- **国际化基础架构** — i18n 多语言支持基础设施就绪，新增英文语言包

### 优化 (Changed)
- **店长反馈区块统一** — 看板反馈区块样式与管理端洞察页对齐，信息层级更清晰
- **洞察列表可折叠** — 管理端建议列表和需关注列表支持展开/折叠
- **行动建议角色化分配** — 会议产生的 action_items 按角色（店长/厨师长）自动分配，移除 daily_aggregation 路径
- **店长/厨师长页面视觉统一** — Pure Float Glass 设计语言覆盖至店长和厨师长全部页面

### 修复 (Fixed)
- **日期选择器遮挡** — 移动端 DatePicker 改用 Portal 渲染，避免父级 transform 上下文遮挡弹层
- **会议流水线稳定性** — 改善错误处理、重试和恢复机制
- **产品反馈 RLS** — 补建 anon 角色的 RLS 策略
- **脏 Q&A 数据过滤** — 利用 diarization 状态过滤未完成说话人分离的低质量数据

## [2.2.0] - 2026-03-04

### 新增 (Added)
- **Pure Float Glass 全站视觉升级** — 玻璃拟态设计语言覆盖全站卡片、导航、按钮，统一视觉风格
- **全站图标统一** — 所有 emoji 替换为 lucide-react SVG 矢量图标
- **录音双通道上传** — 录音同时上传 Supabase Storage 和后端 API，提升上传成功率
- **PWA 自动更新** — 检测到新版本立即刷新，无需手动清缓存
- **AI 查询物化视图** — 替代正则 SQL 重写，提升 AI 智库查询性能和安全性

### 修复 (Fixed)
- **sw.js 缓存** — 禁止 Cloudflare HTTP 缓存 SW 文件，确保每次返回最新版本

## [2.1.4] - 2026-03-04

### 修复 (Fixed)
- **PWA 后台流式泄漏** — 切后台或导航离开时 abort 进行中的 fetch 请求，防止 `reader.read()` 悬挂和连接泄漏
- **普通聊天超时中断** — 多工具查询（3轮×15s）期间后端无数据发送，60s 超时误触发。新增 10s 心跳保活
- **汇报连续重试** — 后端不可用时每次刷新都触发失败请求。新增 5 分钟 sessionStorage 冷却，用户"清空对话"时重置
- **流式滚动卡顿** — 每个 SSE chunk 触发 `scrollIntoView({ behavior: 'smooth' })` 导致高频动画堆叠。改为 rAF + 150ms 节流 + 即时滚动
- **localStorage 遍历安全** — `clearStaleStorage()` 遍历中删除条目可能跳过索引，改为两趟遍历（收集 → 删除）
...
- **汇报超时缩短** — 从 90s 降到 60s，max_tokens 从 3072 降到 1536（实际输出 ~500 tokens）

## [2.1.2] - 2026-03-03

### 修复 (Fixed)
- **AI 智库汇报卡死** — 后端在等待 DeepSeek 响应时（最长 90 秒）不发任何数据，前端 60 秒流超时先触发导致连接中断，页面卡在"正在生成今日汇报"。修复：后端每 10 秒发 heartbeat 保活，前端识别 heartbeat 重置计时器
- **错误状态残留** — 流中断或 AI 错误后 `thinkingStatus` 未清除，可能导致"正在思考"指示器残留。修复：所有错误路径显式清除 `thinkingStatus`
...
### 新增 (Added)
- **客户画像提取** — AI 分析自动提取来源渠道（美团/大众点评/朋友介绍等）和到店频次（新客/回头客/常客），新增 `customer_source` 和 `visit_frequency` 字段
- **画像聚合 API** — `GET /api/dashboard/customer-profile` 返回老客占比、来源分布、门店对比
- **管理端总览老客占比** — 总览页指标网格改为 2×2 布局（满意度/覆盖率/复盘完成/老客占比）
- **管理端洞察画像 Tab** — 洞察页新增「画像」Tab，展示频次分布条、来源渠道排行、门店折叠详情
- **产品洞察入口迁移** — 产品 Tab 从洞察页 Tab 栏移至 UserMenu 下拉菜单（仅 superadmin 可见）
...
### 新增 (Added)
- **版本更新弹窗** — 每次版本更新后首次打开自动弹出更新说明，按角色过滤展示相关内容
- **使用指南页面** `/guide` — 查看所有历史版本更新，按角色过滤，最新版本默认展开
- **UserMenu 使用指南入口** — 头像下拉菜单新增「使用指南」，带红点提醒未读更新
- **红点提醒机制** — 弹窗关闭后红点仍在，直到用户主动打开使用指南页面才消失
- **版本更新数据** `release-notes.ts` — 回填 v1.4.0 ~ v2.0.2 共 14 个版本的角色化更新说明
