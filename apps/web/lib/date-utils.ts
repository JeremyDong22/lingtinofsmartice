...
const SV_LOCALE = 'sv-SE'; // outputs YYYY-MM-DD format

// Calculate date string (YYYY-MM-DD) based on selection label (China timezone)
export function getDateForSelection(selection: string): string {
  return selection === '昨日' ? getChinaYesterday() : getChinaToday();
}

// Get China timezone (UTC+8) today as YYYY-MM-DD
export function getChinaToday(): string {
  return new Date().toLocaleDateString(SV_LOCALE, { timeZone: CHINA_TZ });
}

// Get China timezone yesterday as YYYY-MM-DD
export function getChinaYesterday(): string {
  return shiftDate(getChinaToday(), -1);
}
...
export function shiftDate(dateStr: string, days: number): string {
  ...
  return d.toLocaleDateString(SV_LOCALE, { timeZone: 'UTC' });
}
...
export function getChinaDaysAgo(n: number): string {
  return shiftDate(getChinaToday(), -n);
}
